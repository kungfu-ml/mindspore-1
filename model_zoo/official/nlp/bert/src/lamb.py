...
def _update_run_op(beta1, beta2, eps, global_step, lr, weight_decay, param, m,
    ...
    name = param.name
    name = name.replace(".", "-")
    np.save("./gradients/{}.npy".format(name), gradient.asnumpy())
    """
    Update parameters.
...
@_lamb_opt_ascend.register("Tensor", "Tensor", "Tensor", "Tensor", "Tensor",
                           "Number", "Tensor", "Tensor", "Tensor", "Tensor",
                           "Bool", "Bool")
...
class Lamb(Optimizer):
    ...
    def construct(self, gradients):
        ...
        if self.is_group:
            if self.is_group_lr:
                optim_result = self.hyper_map(
                    F.partial(lamb_opt, self.beta1, self.beta2, self.eps,
        ...
        if self.use_parallel:
            optim_result = F.depend(optim_result,
                                    self.broadcast_params(optim_result))

        if not self.dynamic_lr:
            optim_result = F.depend(optim_result,
                                    self.assignadd(self.global_step, 1))
