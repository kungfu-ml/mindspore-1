...
from mindspore.nn.optim import THOR, AdamWeightDecay, Lamb, Momentum
...
from src import (AdamWeightDecayForBert, BertNetworkWithLoss,
...
from src.utils import BertLearningRate, LossCallBack
...
def _get_optimizer(args_opt, network):
    ...
    if cfg.optimizer == 'Lamb':
        ...
        if args_opt.enable_lossscale == "true" and args_opt.device_target == 'GPU':
            optimizer = AdamWeightDecayForBert(group_params,
                                               learning_rate=lr_schedule,
        ...
        from src.utils import get_bert_thor_damping, get_bert_thor_lr
        lr = get_bert_thor_lr(cfg.Thor.lr_max, cfg.Thor.lr_min,
                              cfg.Thor.lr_power, cfg.Thor.lr_total_steps)
        damping = get_bert_thor_damping(cfg.Thor.damping_max,
                                        cfg.Thor.damping_min,
                                        cfg.Thor.damping_power,
        ...
        optimizer = THOR(network,
                         lr,
                         damping,
                         cfg.Thor.momentum,
                         cfg.Thor.weight_decay,
                         cfg.Thor.loss_scale,
                         cfg.batch_size,
                         decay_filter=lambda x: 'layernorm' not in x.name.
                         lower() and 'bias' not in x.name.lower(),
...
def _set_graph_kernel_context(device_target, enable_graph_kernel,
    ...
    if enable_graph_kernel == "true" or is_auto_enable_graph_kernel:
        if device_target == 'GPU':
            ...
            logger.warning(
                'Graph kernel only supports GPU back-end now, run with graph kernel off.'
            )
...
def run_pretrain():
    ...
    save_checkpoint(model._train_network,
                    os.path.join(ckpt_save_dir, "init-train-net.ckpt"))
